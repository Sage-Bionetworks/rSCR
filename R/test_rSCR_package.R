.test <- function(dir=system.file("unitTests", package="rSCR"), testFileRegexp = "^test_.*\\.R$") {
...
.runTestSuite <- function(dir, testFileRegexp, testFuncRegexp, suiteName) {
        ...
        suite <- defineTestSuite(name=paste("rSCR RUnit Test Suite", suiteName),
                        dirs=dir,
        ...
        if (length(details <- .failure_details(result)) >0) {
                ...
                cat("\n\n")
                stop(paste(suiteName, " tests failed for package rSCR"))
        }
        result
}
